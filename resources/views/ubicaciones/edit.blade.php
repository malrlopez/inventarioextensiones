...
@section('title', 'Editar Ubicación - Sistema de Inventario')
            ...
            <form action="{{ route('ubicaciones.update', $ubicacion->id_ubicacion) }}" method="POST">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('id_sede') is-invalid @enderror" id="id_sede" name="id_sede" required>
                            ...
                            @foreach($sedes as $sede)
                                <option value="{{ $sede->id_sede }}" {{ old('id_sede', $ubicacion->id_sede) == $sede->id_sede ? 'selected' : '' }}>
                                    {{ $sede->nombre_sede }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('id_bloque') is-invalid @enderror" id="id_bloque" name="id_bloque" required>
                            ...
                            @foreach($bloques as $bloque)
                                <option value="{{ $bloque->id_bloque }}" data-sede="{{ $bloque->id_sede }}" {{ old('id_bloque', $ubicacion->id_bloque) == $bloque->id_bloque ? 'selected' : '' }}>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('oficina') is-invalid @enderror" id="oficina" name="oficina" value="{{ old('oficina', $ubicacion->oficina) }}" required>
                        @error('oficina')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('planta_telefonica') is-invalid @enderror" id="planta_telefonica" name="planta_telefonica" value="{{ old('planta_telefonica', $ubicacion->planta_telefonica) }}">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('cuarto_tecnico') is-invalid @enderror" id="cuarto_tecnico" name="cuarto_tecnico" value="{{ old('cuarto_tecnico', $ubicacion->cuarto_tecnico) }}">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('rack') is-invalid @enderror" id="rack" name="rack" value="{{ old('rack', $ubicacion->rack) }}">
                        @error('rack')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('patch_panel') is-invalid @enderror" id="patch_panel" name="patch_panel" value="{{ old('patch_panel', $ubicacion->patch_panel) }}">
                        @error('patch_panel')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control @error('faceplate') is-invalid @enderror" id="faceplate" name="faceplate" value="{{ old('faceplate', $ubicacion->faceplate) }}">
                        @error('faceplate')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
...
<script>
    ...
    // Ejecutar al cargar la página para filtrar según el valor inicial
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('id_sede').dispatchEvent(new Event('change'));
    });
</script>
@endsection
